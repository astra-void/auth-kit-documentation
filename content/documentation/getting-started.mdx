...
export { handler as GET, handler as POST };
```
> ⚠️ If no Challenge Store is provided, an in-memory store will be used, which is **not recommended for production**.
...
Head over to the [Examples](/documentation/examples)for complete usage patterns in client and server components.

→ Check out the [Adapters](/documentation/adapters/overview) for advanced usage.